...
public enum Engine {
  ...
  /// `bindings`.
  ///
  /// Each CTE's produced rows must match the arity its column list declares —
  /// the resolver advertises `cte.columns.count` columns, so a row of a
  /// different width would index out of bounds when a later query reads it. A
  /// `SELECT *` body (or a recursive arm) whose width is known only after the
  /// rows are produced is checked here, faulting with `SQLError.columns` rather
  /// than trapping.
  internal static func with<C: Catalog & ~Escapable>(_ ctes: Array<CTE>,
    ...
    for cte in ctes {
      ...
      // the base catalog plus the CTEs done so far.
      let rows = if cte.recursive && recursive(cte) {
        try fixpoint(cte, catalog, relations, routines, bindings)
      } else {
        try run(cte.query, catalog, relations, routines, bindings)
      }
      // The materialised relation advertises `cte.columns.count` columns; a row
      // of any other width would trap on a later read, so reject it here.
      for row in rows where row.count != cte.columns.count {
        throw .columns(expected: cte.columns.count, got: row.count)
      }
  ...
  /// its query names `cte.name` anywhere in a `FROM` or `JOIN`.
  private static func recursive(_ cte: CTE) -> Bool {
    references(cte.query, cte.name.lowercased())
  }

  /// Whether `query` names the relation `name` (case-folded) in any arm's
  /// `FROM` or `JOIN` — walking the left-associative `UNION` chain and each
  /// arm's relations.
  ...
  private static func resolve<C: Catalog & ~Escapable>(_ relation: Relation,
    ...
    if let view = catalog.view(named: name) {
      let plan = try compile(view.query, catalog, ctes)
  ...
  internal static func optimise<C: Catalog & ~Escapable>(_ plan: Plan,
    ...
    case let .derived(name, plan, ordinals, seek):
      ...
      try .derived(name: name, plan: optimise(plan, catalog, ctes, bindings),
