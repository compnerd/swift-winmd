...
public struct WinMDRelation: SQLEngine.Table, ~Escapable {
...
public struct WinMDCursor: SQLEngine.Cursor, ~Escapable {
...
/// null.
public struct WinMDRow: SQLEngine.Row, ~Escapable {
...
extension WinMD.Storage {
  ...
  package borrowing func identities(ofMethod method: Int) -> Set<Referent> {
  ...
  ///
  /// The field sibling of `identities(ofMethod:)`: it builds a `Resolver` over
  /// the field's `declaration` signature and returns its references, the empty
  /// set on any undecodable row.
  package borrowing func identities(ofField field: Int) -> Set<Referent> {
  ...
  package borrowing func value(ofField field: Int) -> Int? {
