...
extension QueryBuilder {
  ...
  /// LINQ `DefaultIfEmpty`. It lowers to `self UNION ALL (SELECT <values> FROM
  /// (VALUES (0)) AS defaults WHERE NOT EXISTS (self))`: the ISO `<table value
  /// constructor>` `VALUES (0)` is a one-row derived table giving the guard arm
  /// a `FROM` to hang its `WHERE` on, so the arm contributes its single row
  /// exactly when `self` is empty. (A FROM-less `SELECT` cannot carry a
  /// `WHERE`, and is not ISO regardless, so `VALUES` is the one-row source.)
  /// `values` must match this query's projection in width and order — a `UNION
  /// ALL` pairs the arms' columns positionally — so a `defaultIfEmpty` over a
  /// `SELECT *` must spell each column's default.
  public func `default`(_ values: any TermConvertible...) -> SetQuery {
    ...
    // `WHERE` on (a FROM-less `SELECT` is neither ISO nor admitted by the
    // engine, so `VALUES` is the one-row source).
    ...
    let arm = Select(distinct: false, projection: .expressions(defaults),
                     ...
                     joins: [], predicate: .exists(query, negated: true),
                     grouping: .keys([]), having: nil, order: nil, limit: nil)
    return SetQuery(.setop(.union, query, .select(arm), all: true))
  }
}
