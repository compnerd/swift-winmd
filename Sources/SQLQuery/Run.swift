...
extension QueryBuilder {
  ...
  /// Whether every source row satisfies `predicate` — LINQ `All`. It is the SQL
  /// `NOT EXISTS (… WHERE NOT predicate)`: TRUE when no row falsifies the
  /// predicate, so an empty source is vacuously TRUE and a row whose predicate
  /// is UNKNOWN (a NULL) is not a violation. It conjoins `predicate` with any
  /// `where(_:)` already set — a row can violate only among those the chain's
  /// own filter keeps — then tests for the absence of a violating row.
  public borrowing func all<C>(_ predicate: Filter,
                               against catalog: borrowing C,
                               routines: Routines, bindings: Bindings = [:])
      throws(SQLError) -> Bool where C: Catalog & ~Escapable {
    try !filtering(by: !predicate).any(against: catalog, routines: routines,
                                       bindings: bindings)
  }
}
