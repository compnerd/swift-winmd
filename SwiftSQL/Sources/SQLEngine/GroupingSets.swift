...
internal func decompose(windowed select: Select,
  ...
  // Whether the sets genuinely super-aggregate — distinct once each key is
  // canonicalized (qualifier-stripped, case-folded), so some arm omits a key
  // another carries. A single set, or several that canonicalize equal
  // (`((dept), (DEPT))`), collapses to one entry and is not super-aggregating.
  // It gates the two in-place arm-lift sites, whose lift traps on the omitted
  // key of the grand-total `()` arm.
  let superaggregating =
      Set(sets.map { Set($0.map(\.canonical)) }).count > 1
  var lift = Lift(routines, keys: keys, members: members, schemas: schemas,
                  superaggregating: superaggregating)
...
private struct Lift {
  ...
  /// canonicalized (a single set, or several equal after canonicalization, is
  /// `false`). It gates the two in-place arm-lift sites: lifting a whole self-
  /// correlating expression (a stalled `.case`, a host-nil `.subquery`) in
  /// place binds its internal correlation in each arm's grouped scope, but the
  ...
  /// Whether a predicate subquery's host blocked (an undecidable bare group key
  /// over an opaque local), signalling the enclosing groundable expression to
  /// arm-lift whole. Distinct from `blocked`, which is host-internal and reset
  /// at each `host` entry: a whole predicate carries no `*gwN` slot to arm-lift
  /// in place, so the block propagates out through this flag to the nearest
  /// window-free `CASE`, arm-lifted whole where its grouped arm scope binds the
  /// bare key — the resolution a scalar `.subquery` reaches directly, arm-
  /// lifting itself in place on the same block. Set by the blocked
  /// `.exists`/`.within`/`.quantified` predicate cases and returned unchanged,
  /// their internal rewrite moot; caught and reset by the `.case` expression
  /// rewrite that discards the recursed branches and arm-lifts around them.
  ...
  /// output by ISO output-alias/ordinal precedence when that output exists — a
  /// local output reference, not a correlation. An unqualified name present in
  /// `surface` (the carrier body's output names) rides verbatim and never
  /// blocks; a name the surface does not expose is no local output, so a bare
  /// group key falls through to the outer correlation exactly as the engine's
  /// carrier resolution falls an absent sort key through to the enclosing
  /// scope. Set while a carrier's `ORDER BY` is rewritten, reset false when the
  /// walk crosses into a nested subquery body (via `rewrite(_ query:)`), whose
  /// own unqualified references follow the ordinary rule.
  private var riding = false

  /// The carrier body's set-op output names while `riding`, or `nil` when they
  /// cannot be derived (a `SELECT *` arm, `outputs(of query:)` returning `nil`)
  /// — then a riding name rides verbatim, the prior conservative behaviour, as
  /// no output surface is known to correlate it against. A determinate surface
  /// rides a present name and correlates an absent group key. Threaded from
  /// `rewrite(_ query:)`, saved and restored across nested contexts alongside
  /// `riding`.
  private var surface: Set<String>? = nil
  ...
  fileprivate init(_ routines: Routines, keys: Set<String>,
    ...
    self.superaggregating = superaggregating
  }
  ...
  fileprivate mutating func substitute(_ expression: Expression)
    ...
    case let .subquery(query):
      ...
      // The subquery cannot host (its bare group key is undecidable over an
      // opaque local), so it would arm-lift in place. Under genuinely super-
      // aggregating sets that lift traps — the grand-total `()` arm carries no
      // value for the omitted key its internal correlation reads — so fault
      // cleanly rather than trap. A single set or all-identical sets arm-lift
      // as before, every arm carrying the keys.
      guard !superaggregating else {
        throw .state("0A000",
                     "a correlated subquery over an opaque local with " +
    ...
    case let .case(whens, otherwise):
      ...
      // arm-lift this window-free CASE whole to one grounded slot, where the
      // arm's grouped scope binds the bare key — the ordinary GROUP BY twin
      // correlates it the same way. The stall save/reset isolates a nested
      // CASE that catches its own stall from an enclosing one. The leaf
      // accumulators are snapshot too and restored on stall, mirroring `host`:
      // an earlier reachable branch may push a real column before a later guard
      // stalls, and arm-lifting the whole CASE to one slot must discard those
      // speculative leaves, else an earlier unreachable branch's scalar rides a
      // surviving column and runs unconditionally.
      let outer = stalled
      stalled = false
      ...
      if stalled {
        ...
        // so fault cleanly. A single set or all-identical sets arm-lift as
        // before, every arm carrying the keys.
        guard !superaggregating else {
          throw .state("0A000",
                       "a correlated predicate over an opaque local with " +
                       "GROUPING SETS is not yet supported")
        }
        return reference(expression)
      }
  ...
  private mutating func rewrite(_ query: Query, bound: Locals) -> Query {
    ...
    // The carrier `ORDER BY` rides above this query body's output surface, so
    // an unqualified sort key binds a body output when the surface exposes it
    // and correlates the outer key when it does not — the body's set-op
    // spellable output names (the left arm's projection, via `bindable(of
    // query:)`) decide. A synthesized `column N` header is excluded: a carrier
    // `ORDER BY "column N"` faults rather than binds, so it must correlate to
    // its `*gwN` union column, not ride a name it cannot spell.
    let names = bindable(of: query)
    let carriers = query.carriers.map {
      rewrite($0, bound: bound, surface: names)
    }
    riding = saved
    surface = outer
  ...
  /// output `surface` — the row operators riding above the body. An unqualified
  /// key the surface exposes binds that local output (output-alias/ordinal
  /// precedence), left verbatim and never blocking; an unqualified group key
  /// the surface does not expose is no local output, so it correlates to its
  /// `*gwN` union column, matching the engine falling an absent sort key
  /// through to the enclosing scope. A reference qualified by a non-local alias
  /// whose bare name is a group key is likewise the correlation, and an ordinal
  /// is carried through. `DISTINCT`/`OFFSET`·`FETCH`/`generated` hold no column
  ...
  private mutating func rewrite(_ select: Select, bound: Locals)
    ...
    let locals = prefix
    // This Select's own `ORDER BY` resolves a bare unqualified name against its
    // projection output names first (ISO output-alias precedence, as
    // `Scope.order` does), so a `U.v AS dept` output ordered by a bare `dept`
    // binds that output rather than being mis-rewritten to the outer group key
    // it shares a name with. A qualified name, or one no output exposes, still
    // rewrites normally. The carrier `riding` path already leaves such names
    // verbatim, so this only adds the missing locality for a hosted body's own
    // order.
    var ordering = locals
    ordering.names.formUnion(outputs(of: select))
    ...
    return Select(distinct: select.distinct,
                  ...
                  window: window, order: rewrite(select.order, bound: ordering),
                  limit: select.limit)
  }
  ...
  /// normalises the projection to its `Projected` items — an `expressions`
  /// list verbatim, a bare-column `columns` list lifted into `Projected`s, a
  /// `*` (`all`) none — then reads each item's output name under that rule: a
  /// non-aggregate select takes `Projected.alias` (an explicit `AS` alias only,
  /// the resolver's `Projection.outputs` rule), an aggregate select takes
  /// `Projected.name` (an alias, else a bare column's name), so a grouped query
  /// orders by a bare group key it projects whether the parser emitted the list
  /// as `columns` or, forced by a sibling `AS`, as `expressions`. A `*` yields
  /// none on either branch — a grouped `SELECT *` faults in `Grouped.terms`,
  /// exposing no group-column name — and an unnamed computed item carries no
  /// `Projected.name`, so its synthesized `column N` positional header names a
  /// derived-table column for membership (`outputs(of query:)`), never an
  /// `ORDER BY` alias, on either branch.
  private func outputs(of select: Select) -> Set<String> {
    let items: Array<Projected> = switch select.projection {
    case let .expressions(list):
      list
    case let .columns(columns):
    ...
    let output: KeyPath<Projected, String?> =
        select.aggregates ? \.name : \.alias
    return Set(items.compactMap { $0[keyPath: output]?.lowercased() })
  }
  ...
  private func outputs(of query: Query) -> Set<String>? {
    ...
    case let .select(select):
      ...
      case let .expressions(items):
        return Set(items.enumerated().map { offset, item in
          (item.name ?? "column \(offset + 1)").lowercased()
        })
      }
  ...
  /// The bindable (case-folded) output names of `query` — the spellable
  /// surface a set-op carrier's `ORDER BY` may name and ride. It mirrors
  /// `outputs(of query:)` but drops every synthesized name a carrier order
  /// cannot bind: an unnamed projection item contributes no `column N`
  /// positional header (a carrier `ORDER BY "column N"` faults rather than
  ...
  /// the name the ordinary grouped form exposes, not the union column. An
  /// `.expressions` item over an unaliased bare column carries the same output
  /// name (`Projected.name` is the column's own name), so when its column
  /// rewrites to a `*gwN` reference it too keeps that name as an alias.
  private mutating func rewrite(_ projection: Projection, bound: Locals)
    ...
    case .all:
      return .all
    case let .expressions(items):
      return .expressions(items.map { item in
        let rewritten = rewrite(item.expression, bound: bound)
        // A bare unaliased column names its own name; when the rewrite changes
        // its identity to a `*gwN` reference, keep that name as an explicit
        // alias so the output schema still exposes it, mirroring the `.columns`
        // branch below.
        if item.alias == nil, case let .column(column) = item.expression,
            rewritten != item.expression {
          return Projected(expression: rewritten, alias: column.name)
        }
        return Projected(expression: rewritten, alias: item.alias)
      })
    case let .columns(columns):
  ...
  private mutating func rewrite(_ expression: Expression, bound: Locals)
    ...
    case let .column(column):
      ...
      guard let qualifier = column.qualifier else {
        ...
        // ISO output-alias/ordinal precedence when the carrier's output
        // `surface` exposes the name — a local output, left verbatim. A name
        // the surface does not expose is no local output: a group key
        // correlates to its `*gwN` union column (the engine falls an absent
        // sort key through to the enclosing scope), a non-key name stays
        // verbatim. An underivable surface rides verbatim, the conservative
        // prior behaviour. Off the carrier path, decide by inner-scope
        ...
        if riding {
          ...
          guard keys.contains(name) else { return expression }
  ...
  fileprivate mutating func substitute(_ predicate: Predicate)
    ...
    case let .exists(query, negated):
      ...
      // returns `nil`) stalls the walk, returning the predicate unchanged, and
      // the enclosing window-free expression arm-lifts whole (`stalled`).
      guard let hosted = host(query) else {
        stalled = true
        return predicate
      }
      return .exists(hosted, negated: negated)
    case let .within(left, query, negated):
      ...
      // substitute them (`dept IN (…)` → `*gw0 IN (…)`). An unhostable subquery
      // stalls the walk unchanged, its row substitution moot, for the enclosing
      // expression to arm-lift whole.
      guard let hosted = host(query) else {
        stalled = true
        return predicate
      }
      return try .within(substitute(left), hosted, negated: negated)
    case let .quantified(left, op, quantifier, query):
      // `(l…) op {ANY|ALL} (subquery)` — as `within`: host the subquery whole
      // (rewriting its qualified-free correlations) and substitute the row, or
      // stall unchanged when it is unhostable for the enclosing expression to
      // arm-lift whole.
      guard let hosted = host(query) else {
        stalled = true
        return predicate
      }
      return try .quantified(substitute(left), op, quantifier, hosted)
    }
  }
