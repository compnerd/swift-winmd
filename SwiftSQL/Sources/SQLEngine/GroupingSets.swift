...
internal func decompose(windowed select: Select,
  ...
  // The whole member expressions each grouping set names — a computed key like
  // `A + 1` as its own expression, not decomposed to its `A` column — each by
  // the resolved identity grouped lowering matches a key by, approximated pre-
  // scope as its `canonical` (qualifier-stripped, case-folded). The lifter
  // matches an outer subexpression's `canonical` against these to lift a whole
  // grouping key to one arm column, so the arm projects the exact key it groups
  // on rather than a bare constituent the grouped scope rejects. Canonical (not
  // raw `Expression` equality) collapses a qualification- or case-variant
  // spelling (`A + 1` ≡ `T.A + 1` ≡ `a + 1`) as the arm's `Grouped.term` will.
  ...
  var lift = Lift(routines, keys: keys, members: members)
...
private struct Lift {
  ...
  /// The whole member expressions the grouping sets name — a computed key like
  /// `A + 1` as its own expression. A subexpression equal to a member lifts
  /// whole to one arm column, so the arm projects the exact key it groups on
  /// rather than descending to a bare constituent the grouped scope rejects.
  ...
  fileprivate mutating func substitute(_ expression: Expression)
    ...
    // A whole grouping-set member — a computed key like `A + 1` — lifts whole
    // to one arm column before the walk descends, so the arm projects the exact
    // key it groups on rather than a bare constituent (`A`) the grouped scope
    // rejects. Matched by `canonical` (qualifier-stripped, case-folded), so a
    // projection spelled differently than the key (`A + 1` for a `T.A + 1` key)
    // still lifts whole rather than descending to a bare operand. A literal
    // member reads the same twice and stays inline. A bare column that is
    // itself a member matches here first and still lifts whole.
  ...
  ///
  /// A subquery-bearing form keeps its nested `query` in place — the union-
  /// scope `Resolution` the outer layer resolves against hosts the guard's
  /// subquery — but its grounded left operands still substitute: a `within`
  /// or `quantified` left row (a group-key atom such as `dept IN (…)`) becomes
  /// its `*gwN` arm reference, so the guard reads the arm-projected value.
  /// `exists` names no left operand and so rebuilds unchanged.
  fileprivate mutating func substitute(_ predicate: Predicate)
    ...
    case let .within(left, query, negated):
      return try .within(substitute(left), query, negated: negated)
    case let .quantified(left, op, quantifier, query):
      return try .quantified(substitute(left), op, quantifier, query)
    case .exists:
      // `EXISTS (Q)` names no left operand; its subquery is hosted whole by
      // the outer layer's union-scope `Resolution`, so rebuild it unchanged.
      return predicate
    }
  }
