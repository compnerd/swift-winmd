...
struct WindowOverGroupingSetsTests {
  ...
  @Test func `a correlated EXISTS guard hosts outer over the union`() throws {
    // Finding F (regression): a projection `CASE` guarded by a correlated
    // `EXISTS` — `EXISTS (SELECT 1 FROM U WHERE U.k = T.Id)` names the
    // enclosing group key `T.Id`. `host` rewrites that correlation to its
    // `*gwN` union column and hosts the guard in the outer layer, evaluating it
    // once per output row against the union scope. Id 1 and Id 2 have children
    // (guard 1), Id 3 none (guard 0), each numbered, matching the ordinary
    // `GROUP BY T.Id` form on both run and validate.
    let sets = "SELECT CASE WHEN EXISTS (" +
               "SELECT 1 FROM U WHERE U.k = T.Id) THEN 1 ELSE 0 END, " +
               "ROW_NUMBER() OVER () FROM T GROUP BY GROUPING SETS ((T.Id))"
    try correlated().expect(sets, yields: [[1, 1], [1, 2], [0, 3]])
    #expect(try correlated().columns(of: parse(query: sets), validate: true)
                .count == 2)
    let plain = "SELECT CASE WHEN EXISTS (" +
                "SELECT 1 FROM U WHERE U.k = T.Id) THEN 1 ELSE 0 END, " +
                "ROW_NUMBER() OVER () FROM T GROUP BY T.Id"
  ...
  @Test func `a set-op carrier ORDER BY a bare outer key hosts outer`() throws {
    // Finding G: a hosted set-op scalar subquery whose carrier `ORDER BY` names
    // the outer group key by a bare `Id`, absent from the set-op output surface
    // `{m}` (the left arm's projection). Riding above the `UNION`, a bare key
    // binds a set-op output when the surface exposes it; `Id` is
    // no output here, so it is the outer correlation — the engine falls an
    // absent carrier sort key through to the enclosing scope. The lifter now
    // rewrites the bare key to its `*gwN` union column; leaving every
    // unqualified name verbatim faulted an unknown `Id` over the `*gwN`-only
    // union scope. Both arms take `MAX(U.v)` = 200, so the `UNION` collapses to
    // one row the carrier orders by the correlated key (constant per group) —
    // 200 for every group, each numbered, matching the ordinary `GROUP BY T.Id`
    // form on both run and validate.
    let sets = "SELECT (SELECT MAX(U.v) AS m FROM U UNION " +
               "SELECT MAX(U.v) FROM U ORDER BY Id) AS s, " +
               "ROW_NUMBER() OVER () FROM T GROUP BY GROUPING SETS ((T.Id))"
    try correlated().expect(sets, yields: [[200, 1], [200, 2], [200, 3]])
    #expect(try correlated().columns(of: parse(query: sets), validate: true)
                .count == 2)
    let plain = "SELECT (SELECT MAX(U.v) AS m FROM U UNION " +
                "SELECT MAX(U.v) FROM U ORDER BY Id) AS s, " +
                "ROW_NUMBER() OVER () FROM T GROUP BY T.Id"
    try correlated().expect(plain, yields: [[200, 1], [200, 2], [200, 3]])
  }
  ...
  // A grouped `Emp` beside a `U` whose `v` values are stored out of ascending
  // order, so a subquery ordering by its own `U.v` output picks a different row
  // than the source order (or the constant outer key) would — the observable a
  // hosted subquery's own `ORDER BY` binds its output alias, not the group key.
  ...
  @Test func `a hosted subquery orders by its own output alias`() throws {
    // A hosted scalar subquery `(SELECT U.v AS dept FROM U ORDER BY dept FETCH
    // FIRST 1 ROW ONLY)` names its own output alias `dept` in its `ORDER BY`.
    // That alias collides with the outer grouping key `dept`, but ISO output-
    // alias precedence binds a bare unqualified sort name to the projection
    // output first, so it orders by `U.v` (30, 10, 20 → 10, 20, 30) and takes
    // 10. The subquery is uncorrelated and hosts outer, yielding 10 per group.
    // Left to the FROM/JOIN scope alone the bare `dept` was rewritten to the
    // outer key's `*gwN`, ordering by the constant group value and taking the
    // source-order first row (30) — diverging from the ordinary `GROUP BY dept`
    // form, which binds the output alias.
    let sets = "SELECT (SELECT U.v AS dept FROM U ORDER BY dept " +
               "FETCH FIRST 1 ROW ONLY), ROW_NUMBER() OVER () " +
               "FROM Emp GROUP BY GROUPING SETS ((dept))"
    try sorted().expect(sets, yields: [[10, 1], [10, 2], [10, 3]])
    #expect(try sorted().columns(of: parse(query: sets), validate: true)
                .count == 2)
    let plain = "SELECT (SELECT U.v AS dept FROM U ORDER BY dept " +
                "FETCH FIRST 1 ROW ONLY), ROW_NUMBER() OVER () " +
                "FROM Emp GROUP BY dept"
    try sorted().expect(sets, equals: plain)
    try sorted().expect(plain, yields: [[10, 1], [10, 2], [10, 3]])
  }

  @Test func `a UNION arm keeps its aliased key output name for the carrier`()
      throws {
    // A hosted `EXISTS` subquery whose `UNION` arms project the outer key as a
    // bare `T.Id` beside an aliased `U.v AS x` — a mixed `.expressions` arm —
    // ordered by the carrier `ORDER BY Id`. The lifter rewrites the bare `T.Id`
    // to its `*gwN` union column; the `.expressions` rewrite must alias that
    // reference back to `Id` (its own output name) so the arm still exposes
    // `Id` and the carrier `ORDER BY Id` binds it, mirroring the `.columns`
    // arm. Without the alias the arm output became `*gwN`, so `ORDER BY Id`
    // resolved no output and faulted `.column`. `U` is non-empty for every
    // group, so `EXISTS` holds and the guard is 1, matching the ordinary
    // `GROUP BY T.Id` form.
    let sets = "SELECT CASE WHEN EXISTS (SELECT T.Id, U.v AS x FROM U " +
               "UNION SELECT T.Id, U.v AS x FROM U ORDER BY Id) " +
               "THEN 1 ELSE 0 END, ROW_NUMBER() OVER () " +
               "FROM T GROUP BY GROUPING SETS ((T.Id))"
    try correlated().expect(sets, yields: [[1, 1], [1, 2], [1, 3]])
    #expect(try correlated().columns(of: parse(query: sets), validate: true)
                .count == 2)
    let plain = "SELECT CASE WHEN EXISTS (SELECT T.Id, U.v AS x FROM U " +
                "UNION SELECT T.Id, U.v AS x FROM U ORDER BY Id) " +
                "THEN 1 ELSE 0 END, ROW_NUMBER() OVER () " +
                "FROM T GROUP BY T.Id"
    try correlated().expect(sets, equals: plain)
    try correlated().expect(plain, yields: [[1, 1], [1, 2], [1, 3]])
  }

  @Test func `a hosted self-join ON binds the outer key before its own alias`()
      throws {
    // A hosted subquery joins `U` to itself, its first `ON` naming the outer
    // grouping key `T.Id` while a later join binds its own `U AS T`. Prefix-
    // scoped resolution binds a join `ON` against its preceding relations only,
    // so the first `ON`'s `T.Id` — outside the `U JOIN U AS V` prefix, which
    // exposes no such alias — is the outer correlation, rewritten to its `*gwN`
    // union column even though the chain later introduces a local `T`. Adding
    // every joined relation to the scope before rewriting would let the later
    // `AS T` mask the first `ON`, mis-binding it locally. The count depends on
    // that correlation, matching the ordinary `GROUP BY T.Id` form.
    let sets = "SELECT (SELECT COUNT(*) FROM U JOIN U AS V ON V.k = T.Id " +
               "JOIN U AS T ON T.k = V.k), ROW_NUMBER() OVER () " +
               "FROM T GROUP BY GROUPING SETS ((T.Id))"
    let plain = "SELECT (SELECT COUNT(*) FROM U JOIN U AS V ON V.k = T.Id " +
                "JOIN U AS T ON T.k = V.k), ROW_NUMBER() OVER () " +
                "FROM T GROUP BY T.Id"
    try correlated().expect(sets, equals: plain)
    try correlated().expect(sets, yields: [[12, 1], [3, 2], [0, 3]])
    try correlated().expect(plain, yields: [[12, 1], [3, 2], [0, 3]])
    #expect(try correlated().columns(of: parse(query: sets), validate: true)
                .count == 2)
  }

  @Test func `a derived table's virtual Id binds locally not the group key`()
      throws {
    // F5: `expose` classifies a hosted subquery's FROM relations as local or
    // outer. A `(VALUES (99)) AS v(x)` derived table renames its lone real
    // column to `x`, but the materialised relation still vends the universal
    // virtual `Id`; dropping it under the explicit column list left the
    // subquery's own `Id` mis-classified as an outer correlation and rewritten
    // to the group key. The subquery `(SELECT Id FROM (VALUES (99)) AS v(x))`
    // must read `v`'s own row Id — a constant 1 across every group — not the
    // outer group Id. Emp's five rows each group on their virtual Id, so
    // `ROW_NUMBER() OVER (ORDER BY Id)` numbers them 1..5 while `n` stays 1,
    // matching the ordinary `GROUP BY Id` form on both run and validate; before
    // the fix `n` bound the group Id (1..5).
    let sets = "SELECT ROW_NUMBER() OVER (ORDER BY Id) AS r, " +
               "(SELECT Id FROM (VALUES (99)) AS v(x)) AS n " +
               "FROM Emp GROUP BY GROUPING SETS ((Id))"
    try fixture().expect(sets, yields: [[1, 1], [2, 1], [3, 1], [4, 1], [5, 1]])
    #expect(try fixture().columns(of: parse(query: sets), validate: true)
                .count == 2)
    let plain = "SELECT ROW_NUMBER() OVER (ORDER BY Id) AS r, " +
                "(SELECT Id FROM (VALUES (99)) AS v(x)) AS n " +
  ...
  @Test func `a correlated subquery in an untaken CASE branch stays lazy`()
      throws {
    // A scalar subquery correlated to the group key by `WHERE U.k = T.Id`,
    // dividing by zero, nested in a CASE THEN whose guard `T.Id <> T.Id` is
    // always false — the branch is never taken. Arm-lifting the subquery
    // evaluated it eagerly per group and faulted `.divide`; hosting it inline
    // in the CASE (its `T.Id` rewritten to `*gwN`) keeps it lazy, so the ELSE
    // yields the group key and the division never happens.
    let sets = "SELECT CASE WHEN T.Id <> T.Id " +
               "THEN (SELECT SUM(U.v) / 0 FROM U WHERE U.k = T.Id) " +
               "ELSE T.Id END, ROW_NUMBER() OVER () " +
               "FROM T GROUP BY GROUPING SETS ((T.Id))"
    try correlated().expect(sets, yields: [[1, 1], [2, 2], [3, 3]])
    let plain = "SELECT CASE WHEN T.Id <> T.Id " +
                "THEN (SELECT SUM(U.v) / 0 FROM U WHERE U.k = T.Id) " +
                "ELSE T.Id END, ROW_NUMBER() OVER () FROM T GROUP BY T.Id"
    try correlated().expect(plain, yields: [[1, 1], [2, 2], [3, 3]])
  }

  @Test func `a correlated projection subquery under FETCH 0 stays lazy`()
      throws {
    // A scalar subquery correlated to the group key by `WHERE U.k = T.Id`,
    // dividing by zero, projected under `FETCH FIRST 0 ROWS ONLY`. Arm-lifting
    // it evaluated the division per union row below the cap; hosting it inline
    // above the cap keeps it lazy — the zero-row page drops every row before
    // the projection evaluates, so the result is empty and no division runs.
    let sets = "SELECT (SELECT SUM(U.v) / 0 FROM U WHERE U.k = T.Id), " +
               "ROW_NUMBER() OVER () FROM T GROUP BY GROUPING SETS ((T.Id)) " +
               "FETCH FIRST 0 ROWS ONLY"
    try correlated().expect(sets, yields: [])
    let plain = "SELECT (SELECT SUM(U.v) / 0 FROM U WHERE U.k = T.Id), " +
                "ROW_NUMBER() OVER () FROM T GROUP BY T.Id " +
                "FETCH FIRST 0 ROWS ONLY"
    try correlated().expect(plain, yields: [])
  }
  ...
  @Test func `an opaque local EXISTS guard arm-lifts its window-free CASE`()
      throws {
    // The predicate-stall fix. An `EXISTS` guard's subquery correlates an
    // unqualified `dept` — a group key — through the derived `(SELECT * FROM U)
    // x`. Where the lowering cannot expand that `SELECT *` (its columns not
    // derivable, so `expose` returns nil), `dept` is undecidable and `host`
    // blocks; a whole predicate holds no `*gwN` slot, so the block once hosted
    // the unrewritten subquery, whose bare `dept` faulted `.column` over the
    // `*gwN`-only union scope. The stall now arm-lifts the window-free `CASE`
    // whole, its grouped arm scope binding `dept`, matching the ordinary
    // `GROUP BY dept` twin. Where the lowering can derive the `SELECT *` names
    // instead, the subquery hosts outer and the guard resolves the same, so the
    // result holds either way. `U.v` in {1, 2, 3}: `x.v > dept` yields 1 for
    // `dept` 1 and 2 and 0 for 3, each numbered, on run and validate.
    let sets =
        "SELECT CASE WHEN EXISTS (" +
        "SELECT * FROM (SELECT * FROM U) x WHERE x.v > dept) " +
        "THEN 1 ELSE 0 END, ROW_NUMBER() OVER () " +
        "FROM Emp GROUP BY GROUPING SETS ((dept))"
    try colliding().expect(sets, yields: [[1, 1], [1, 2], [0, 3]])
    #expect(try colliding().columns(of: parse(query: sets), validate: true)
                .count == 2)
    let plain =
        "SELECT CASE WHEN EXISTS (" +
  ...
  @Test func `a derivable local EXISTS guard hosts outer not arm-lifted`()
      throws {
    // The soundness twin: an explicit derived column list makes the local
    // derivable at every slice — `(SELECT * FROM U) AS x(v)` exposes {v, id},
    // no opaque local — so the unqualified `dept`, absent from it, is decided
    // the outer key, rewritten to `*gwN` and hosted outer with no stall, before
    // and after the fix alike. `dept` 1 and 2 find a `U.v` above them (yielding
    // 1), 3 finds none (yielding 0), each numbered, matching the ordinary
    // `GROUP BY dept` companion — proving the stall never over-arm-lifts a
    // derivable guard.
    let sets =
        "SELECT CASE WHEN EXISTS (" +
        "SELECT * FROM (SELECT * FROM U) AS x(v) WHERE x.v > dept) " +
        "THEN 1 ELSE 0 END, ROW_NUMBER() OVER () " +
        "FROM Emp GROUP BY GROUPING SETS ((dept))"
    try colliding().expect(sets, yields: [[1, 1], [1, 2], [0, 3]])
    #expect(try colliding().columns(of: parse(query: sets), validate: true)
  ...
  // the schema derivation synthesizes for an unnamed computed projection, space
  // and all. A derived table's unnamed projection takes that same name, so a
  // subquery over it must bind the local synthesized column rather than the
  // equally spelled outer group key.
  private func spaced() throws -> FixtureCatalog {
    try Catalog {
      Relation("T", ["column 1": .integer]) {
        Row(10)
        Row(20)
      }
      Relation("U", ["v": .integer]) {
  ...
  @Test func `a derived synthesized name binds the local subquery column`()
      throws {
    // The derived table `v` — `(SELECT 99 + 0 FROM T FETCH FIRST 1 ROW ONLY)` —
    // has one unnamed computed projection, which the schema derivation names
    // `column 1` (with the space), the same spelling as the outer group key.
    // The subquery's `"column 1"` must bind v's local synthesized column (99),
    // not the outer group key (10 / 20): `outputs(of select:)` must expose the
    // unnamed item under its synthesized positional name so the host resolves
    // the reference locally. Dropping it left the name unexposed, so the host
    // rewrote it to the outer union column and the subquery returned 10 / 20.
    // Over the single set the group key is `column 1` (10, 20); the subquery is
    // 99 for each; `ROW_NUMBER() OVER ()` numbers the two union rows.
    let sql = """
        SELECT (SELECT "column 1" + 0
                  FROM (SELECT 99 + 0 FROM T FETCH FIRST 1 ROW ONLY) AS v),
               ROW_NUMBER() OVER ()
          FROM T GROUP BY GROUPING SETS (("column 1"))
        """
    try spaced().expect(sql, yields: [[99, 1], [99, 2]])
    #expect(try spaced().columns(of: parse(query: sql), validate: true)
                .count == 2)
    // The ordinary plain-`GROUP BY` windowed twin bypasses the grouping-sets
    // lowering, resolving the subquery through the engine's own scope — the
    // reference oracle the direct lowering must equal.
    let twin = """
        SELECT (SELECT "column 1" + 0
                  FROM (SELECT 99 + 0 FROM T FETCH FIRST 1 ROW ONLY) AS v),
               ROW_NUMBER() OVER ()
          FROM T GROUP BY "column 1"
        """
    try spaced().expect(twin, yields: [[99, 1], [99, 2]])
  }

  @Test func `a synthesized header is not a local ORDER BY alias`() throws {
    // The hosted subquery `SELECT 99 + 0 FROM U ORDER BY "column 1"` projects
    // an unnamed computed item whose synthesized header is `column 1` — the
    // same spelling as the outer group key. Only an explicit `AS` names an
    // `ORDER BY` alias (the `Projection.outputs` rule), so a bare `column 1`
    // is no local output here; it correlates to the outer group key as the
    // ordinary grouped query resolves it, not the subquery's own header.
    // Admitting the synthesized name as a local alias blocked that correlation.
    let sql = """
        SELECT (SELECT 99 + 0 FROM U ORDER BY "column 1"),
               ROW_NUMBER() OVER ()
          FROM T GROUP BY GROUPING SETS (("column 1"))
        """
    try spaced().expect(sql, yields: [[99, 1], [99, 2]])
  ...
  @Test func `a set-op carrier ORDER BY a synthesized header hosts outer`()
      throws {
    // The set-op sibling of the SELECT-local case: the hosted subquery is a
    // `UNION` whose carrier `ORDER BY "column 1"` names the outer group key,
    // and the first arm's `99 + 0` is unnamed — its synthesized header is
    // `column 1`, the same spelling. A carrier order binds only a spellable arm
    // output, so the synthesized header must not expose `column 1`: the key
    // falls through and correlates to its `*gwN` union column, as the ordinary
    // grouped query resolves the outer key. `bindable(of query:)` drops the
    // header the synthesized-inclusive `outputs(of query:)` kept, which rode
    // the key verbatim into the union scope as an unbound `column 1`. Both arms
    // are 99, so the `UNION` collapses to one row and the subquery is 99 per
    // group; over the single set the group key is `column 1` (10, 20), each
    // numbered — matching the ordinary form.
    let sql = """
        SELECT (SELECT 99 + 0 FROM U
                UNION SELECT 99 + 0 FROM U ORDER BY "column 1"),
               ROW_NUMBER() OVER ()
          FROM T GROUP BY GROUPING SETS (("column 1"))
        """
    try spaced().expect(sql, yields: [[99, 1], [99, 2]])
  ...
  @Test func `a set-op carrier ORDER BY a bare arm column rides`() throws {
    // The spellable half of `bindable(of query:)`: a bare projected column is a
    // real arm output, so its name must still ride. The hosted `UNION` projects
    // a bare `v`, and the carrier `ORDER BY v` binds that arm output by ISO
    // output-name precedence — left verbatim, never correlated to a `*gwN`
    // column. Keying the surface off `item.name` (an explicit alias or the bare
    // column's own name) keeps `v` spellable; an alias-only surface would drop
    // it and wrongly correlate. Both arms are `v` = 1, so the subquery is 1 per
    // group; the group key is `column 1` (10, 20), each numbered.
    let sql = """
        SELECT (SELECT v FROM U UNION SELECT v FROM U ORDER BY v),
               ROW_NUMBER() OVER ()
          FROM T GROUP BY GROUPING SETS (("column 1"))
        """
    try spaced().expect(sql, yields: [[1, 1], [1, 2]])
    #expect(try spaced().columns(of: parse(query: sql), validate: true)
                .count == 2)
    let twin = """
        SELECT (SELECT v FROM U UNION SELECT v FROM U ORDER BY v),
          ...
          FROM T GROUP BY "column 1"
        """
    try spaced().expect(twin, yields: [[1, 1], [1, 2]])
  }

  @Test func `a stalled CASE discards an earlier branch's speculative leaf`()
      throws {
    // A window-free `CASE` whose earlier, never-taken branch nests a stateful
    // scalar over an opaque local (`(SELECT * FROM U)`), so its unqualified
    // `dept` blocks hosting and the branch arm-lifts to a `*gwN` leaf, while a
    // later `EXISTS` guard over the same opaque local stalls the walk, so the
    // whole `CASE` arm-lifts to one slot. The stall catch must discard the
    // earlier branch's speculative leaf: left in place it rides its own arm
    // column, evaluating `tick()` eagerly per group though the branch guard
    // `1 = 0` never holds. Rolled back, only the whole-`CASE` slot survives,
    // whose guard skips the branch, so `tick()` never runs — matching the
    // ordinary `GROUP BY dept` twin, which evaluates the `CASE` lazily.
    let counter = Counter()
    let routines = try ticking(counter)
    let branch = "CASE WHEN 1 = 0 THEN " +
                 "(SELECT tick() FROM (SELECT * FROM U) o WHERE o.v = dept) " +
                 "WHEN EXISTS (" +
                 "SELECT * FROM (SELECT * FROM U) x WHERE x.v > dept) THEN 1 " +
                 "ELSE 0 END"
    let sets = "SELECT LEAD(\(branch), 1, 0) OVER (ORDER BY dept) " +
               "FROM Emp GROUP BY GROUPING SETS ((dept))"
    let plain = "SELECT LEAD(\(branch), 1, 0) OVER (ORDER BY dept) " +
                "FROM Emp GROUP BY dept"
    try colliding().expect(sets, equals: plain, routines: routines)
    #expect(counter.count == 0)
    #expect(try colliding().columns(of: parse(query: sets), routines: routines,
                                    validate: true).count == 1)
  }

  // A grouped `Emp` keyed on `v` beside an opaque local `U` also keyed on `v`,
  // so a hosted subquery grouping and ordering by a bare `v` collides that name
  // with the outer group key.
  private func keyed() throws -> FixtureCatalog {
    try Catalog {
      Relation("Emp", ["v": .integer]) {
        Row(1)
        Row(2)
        Row(3)
      }
  ...
  @Test func `a grouped subquery ORDER BY a bare group column binds locally`()
      throws {
    // A `LEAD` default nests `CASE WHEN EXISTS (SELECT d.v, 0 FROM (SELECT *
    // FROM U) d WHERE tick() > 0 GROUP BY d.v ORDER BY v) THEN 1 ELSE 0 END`.
    // The `EXISTS` body's `ORDER BY v` names the bare projected group column
    // `d.v`, whose inferred name collides with the outer group key `v` (a
    // second projected `0` keeps the item list an `expressions` projection
    // rather than collapsing to a bare column list). Over an opaque local the
    // hosted body's own order surface admitted only explicit `AS` outputs, so a
    // bare `v` fell through and correlated outward; undecidable there, `host`
    // blocked, the `EXISTS` stalled, and the whole `CASE` arm-lifted —
    // evaluating `tick()` eagerly per group though offset 0 never reaches the
    // default. An aggregate select's surface also admits an unaliased projected
    // group column (`Grouped.order`), so `v` binds `d.v` locally, the `EXISTS`
    // hosts, the `CASE` stays lazy in the `LEAD` default, and `tick()` never
    // runs — matching the ordinary `GROUP BY v` twin.
    let counter = Counter()
    let routines = try ticking(counter)
    let body = "CASE WHEN EXISTS (SELECT d.v, 0 FROM (SELECT * FROM U) d " +
               "WHERE tick() > 0 GROUP BY d.v ORDER BY v) THEN 1 ELSE 0 END"
    let sets = "SELECT LEAD(v, 0, \(body)) OVER (ORDER BY v) " +
               "FROM Emp GROUP BY GROUPING SETS ((v))"
    let plain = "SELECT LEAD(v, 0, \(body)) OVER (ORDER BY v) " +
                "FROM Emp GROUP BY v"
    try keyed().expect(sets, equals: plain, routines: routines)
    #expect(counter.count == 0)
    #expect(try keyed().columns(of: parse(query: sets), routines: routines,
                                validate: true).count == 1)
  }

  @Test func `a grouped columns subquery orders by its bare group column`()
      throws {
    // The `.columns` sibling of the `.expressions` case above: the `EXISTS`
    // body `SELECT d.v FROM (SELECT * FROM U) d WHERE tick() > 0 GROUP BY d.v
    // ORDER BY v` projects only the bare `d.v`, so the parser emits a `columns`
    // projection, not an `expressions` list. Its `ORDER BY v` names the bare
    // projected group column, whose inferred name collides with the outer group
    // key `v`. The lift's `outputs(of select:)` returned no name for a bare
    // `columns` projection, so `v` was no local output — the reference fell
    // through, correlated outward, `host` blocked, the `EXISTS` stalled, so the
    // `CASE` arm-lifted, evaluating `tick()` eagerly per group though offset 0
    // never reaches the `LEAD` default. Mirroring `Select.orderKeys`, an
    // aggregate select now exposes an unaliased projected group column by its
    // `Projected.name` for a `columns` projection too, so `v` binds `d.v`
    // locally, the `EXISTS` hosts, the `CASE` stays lazy, `tick()` never runs,
    // and the result matches the ordinary `GROUP BY v` twin.
    let counter = Counter()
    let routines = try ticking(counter)
    let body = "CASE WHEN EXISTS (SELECT d.v FROM (SELECT * FROM U) d " +
               "WHERE tick() > 0 GROUP BY d.v ORDER BY v) THEN 1 ELSE 0 END"
    let sets = "SELECT LEAD(v, 0, \(body)) OVER (ORDER BY v) " +
               "FROM Emp GROUP BY GROUPING SETS ((v))"
    let plain = "SELECT LEAD(v, 0, \(body)) OVER (ORDER BY v) " +
  ...
  @Test func `a window operand is validated under a zero fetch cap`() throws {
    // The window node lifted over the union sits below the FETCH cap (window
    // under sort under limit), so it evaluates its operands over every union
    // row before the page drops them. `SUM('x' + 1) OVER ()` folds text and an
    // integer, faulting `42804` at run — yet the projection wrapper above the
    // cap is unreachable under `FETCH FIRST 0 ROWS`, so the schema derive once
    // skipped it and `columns(of:validate:)` accepted, the cardinal run ≢
    // validate divergence. Validating the window operands unconditionally
    // (independent of the wrapper's reachability) makes validate fault
    // identically.
    let sql = "SELECT SUM('x' + 1) OVER () FROM Emp " +
              "GROUP BY GROUPING SETS (()) FETCH FIRST 0 ROWS ONLY"
    try fixture().expect(sql, fails: .operand("operands must be numeric"))
    #expect(throws: SQLError.operand("operands must be numeric")) {
      _ = try fixture().columns(of: parse(query: sql), validate: true)
    }
    // The companion: a well-typed window over the grand-total aggregate under
    // the same zero cap must not be over-rejected — it validates to its one
    // column and runs to no rows, the above-cap wrapper staying legitimately
    // unreachable.
    let valid = "SELECT SUM(SUM(sal)) OVER () FROM Emp " +
                "GROUP BY GROUPING SETS (()) FETCH FIRST 0 ROWS ONLY"
    #expect(try fixture().columns(of: parse(query: valid), validate: true)
                .count == 1)
    try fixture().empty(valid)
  }

  @Test func `a hosted subquery over an engine-provided view binds locally`()
      throws {
    // The membership map that decides whether a hosted subquery's relations
    // bind locally seeds the engine-provided `information_schema.` views beside
    // the catalog's own. A scalar subquery correlating the outer group key
    // `dept` over `information_schema.tables` — a source no catalog registers —
    // once found the view an opaque local: `dept` could hide in it, so the
    // rewrite blocked and the subquery arm-lifted in place, trapping on the
    // grand-total `()` arm that omits `dept` and faulting `0A000`. With the
    // standard view in the map the view is a determinate local `dept` is absent
    // from, so `dept` is the outer key, rewritten to its union column and
    // hosted: each per-`dept` arm projects its own `dept`, the `()` arm the
    // super-aggregate NULL, numbered across the union.
    let sql = "SELECT (SELECT dept FROM information_schema.tables " +
              "FETCH FIRST 1 ROW ONLY), ROW_NUMBER() OVER () " +
              "FROM Emp GROUP BY GROUPING SETS ((dept), ())"
